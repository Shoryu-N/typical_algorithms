...
#(1 << n)で初期化すると、n=5のとき2**5=32になる。つまり、indexは0~31(二進数で00000~11111)をさせる
...
for i in range(n):
    ...
    e.append((x,y,z))
#下記実装でi==jのときは0が入り、行きと帰りで重みが違っても適切に入る    
...
#n = 5なら2**n-1のbitは11111。全部立っている状態で、最後に0に向かう時のコストがこたえ
print(dp[2**n-1][0])
